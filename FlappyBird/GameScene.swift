...
class GameScene: SKScene, SKPhysicsContactDelegate  {
 ...
 // SKView上にシーンが表示されたときに呼ばれるメソッド
    override func didMove(to view: SKView) {
        ...
        playSound = SKAction.playSoundFileNamed("mens-yeah1", waitForCompletion: true)
    ...
    func setupItem(){
        // アイテムの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "item_1")
        itemTexture.filteringMode = .linear
        
        //x, yの位置設定用に地面の画像サイズを取得
        let groundSize = SKTexture(imageNamed: "ground").size()
        
        let ground_y = groundSize.height * 2
        
        let ground_x = groundSize.width * 2

        
        // 移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + itemTexture.size().width + ground_x / 2)

        // 画面外まで移動するアクションを作成
        let moveItem = SKAction.moveBy(x:  -1.05 * movingDistance, y: 0, duration:7.5)
        ...
        let createItemAnimation = SKAction.run({
            ...
            let item_y = ground_y + random_y

             
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2 + 2.2 * ground_x / 5, y: item_y)
            item.zPosition = -30 // 雲より手前、地面より奥
    ...
    func setupScoreLabel() {
        ...
        scoreLabelNode.text = "Score:\(score) ItemScore:\(itemscore)"
